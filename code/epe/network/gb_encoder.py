...
class GBufferEncoder(nn.Module):
	...
	def _make_gbuffer_encoders(self):
		...
		for i in range(self.num_classes):
			dim_in = self.num_gbuffer_channels if i not in cls2dim else cls2dim[i]
	...
	def forward(self, gbuffers, classmap):
		...
		features = sum(
			classmap[:, c, :, :]
			* self.class_encoders[c](
				self.cls2gbuf[c](gbuffers) if c in self.cls2gbuf else gbuffers
			)
			for c in range(num_classes)
		)
		features = [features]
		features.extend(layer(features[-1]) for layer in self.joint_encoder_layers)
		return features[1:]
